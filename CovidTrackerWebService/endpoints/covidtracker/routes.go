...
func (C CovidData) SetRoutes(router *gin.Engine) {
	router.GET("/allData", C.GetAllData)
	router.GET("/data/:date", C.ReadData)
	router.POST("/data", C.CreateData)
	router.PUT("/data/:date", C.UpdateData)
	router.DELETE("/data/:date", C.DeleteData)
}
